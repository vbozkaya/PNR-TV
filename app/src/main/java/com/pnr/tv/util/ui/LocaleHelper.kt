...
object LocaleHelper {
    ...
    fun getDeviceLanguageWithRegion(context: Context): String {
        ...
        return if (country.isNotEmpty()) {
            ...
            // Country boşsa, dil koduna göre standart bölge kullan
            when (language) {
                "en" -> "en-US" // İngilizce için ABD
                "tr" -> "tr-TR" // Türkçe için Türkiye
                "pt" -> "pt-BR" // Portekizce için Brezilya (en yaygın)
                "fr" -> "fr-FR" // Fransızca için Fransa
                "es" -> "es-ES" // İspanyolca için İspanya
                "hi" -> "hi-IN" // Hintçe için Hindistan
                "id" -> "id-ID" // Endonezce için Endonezya
                "ja" -> "ja-JP" // Japonca için Japonya
                "th" -> "th-TH" // Tayca için Tayland
                else -> language // Diğer diller için sadece dil kodu (TMDB kabul eder)
            }
        }
    }
