...
class PlayerControlView @JvmOverloads constructor(
    ...
    fun showControls() {
        ...
        timber.log.Timber.d("🔍 Visibility ayarlandı: parent=${visibility}, top=${binding.panelTopSection.visibility}, bottom=${binding.panelBottomSection.visibility}")
        ...
        post {
            ...
            timber.log.Timber.d("  - panelTopSection: ${binding.panelTopSection.width}x${binding.panelTopSection.height}, visibility=${binding.panelTopSection.visibility}")
            timber.log.Timber.d("  - panelBottomSection: ${binding.panelBottomSection.width}x${binding.panelBottomSection.height}, visibility=${binding.panelBottomSection.visibility}")
            timber.log.Timber.d("  - txtContentTitle: ${binding.txtContentTitle.width}x${binding.txtContentTitle.height}, visibility=${binding.txtContentTitle.visibility}, text='${binding.txtContentTitle.text}'")
            timber.log.Timber.d("  - seekbarProgress: ${binding.seekbarProgress.width}x${binding.seekbarProgress.height}, visibility=${binding.seekbarProgress.visibility}")
            timber.log.Timber.d("  - btnPlay: ${binding.btnPlay.width}x${binding.btnPlay.height}, visibility=${binding.btnPlay.visibility}")
            timber.log.Timber.d("  - btnStop: ${binding.btnStop.width}x${binding.btnStop.height}, visibility=${binding.btnStop.visibility}")
    ...
    override fun dispatchKeyEvent(event: KeyEvent?): Boolean {
        ...
        when (event.action) {
            KeyEvent.ACTION_DOWN -> {
                ...
                if (event.keyCode == KeyEvent.KEYCODE_DPAD_LEFT && focusedView == binding.btnBackward) {
                    ...
                    if (targetButton.visibility == View.VISIBLE) {
                        targetButton.requestFocus()
                        timber.log.Timber.d("🎯 Backward'dan ${if (listener?.isPlayingState() == true) "Stop" else "Play"} butonuna focus")
                        return true
                    }
                }
            }
            KeyEvent.ACTION_UP -> {
                ...
                val newFocusedViewName = when (newFocusedView) {
                    binding.seekbarProgress -> "SeekBar"
                    binding.btnPlay -> "Play"
                    binding.btnStop -> "Stop"
                    binding.btnBackward -> "Backward"
        ...
        val finalFocusedViewName = when (finalFocusedView) {
            binding.seekbarProgress -> "SeekBar"
            binding.btnPlay -> "Play"
            binding.btnStop -> "Stop"
            binding.btnBackward -> "Backward"
    ...
    private fun startSeekCommitTimer(newPositionMs: Long) {
        ...
        seekCommitJob = viewScope?.launch {
    ...
    private fun setupPlayStopButtons() {
        ...
        binding.btnPlay.setOnClickListener {
            ...
            timber.log.Timber.d("▶️▶️▶️ PLAY BUTONU onClick TETİKLENDİ! Focus=${currentFocus?.javaClass?.simpleName}, isInitialFocus=$isInitialFocus")
    ...
    private fun handleSeekButtonClick(amount: Long) {
        ...
        timber.log.Timber.d("🔘 handleSeekButtonClick başladı: amount=${amount/1000}s, focus önce=${focusBefore?.javaClass?.simpleName}")
        ...
        timber.log.Timber.d("🔘 handleSeekButtonClick: currentPosition=${currentPosition/1000}s, newPosition=${newPosition/1000}s, newProgress=$newProgress, post bloğu çağrılıyor...")
    ...
    private fun resetAutoHideTimer() {
        ...
        // Yeni sayaç başlat
        hideControlsJob = viewScope?.launch {
